...
from database import (
    init_db, save_task, update_task, confirm_task, get_task, get_tasks, publish_task,
...
def render_business():
    ...
    statuses = {"pending": "Ожидает решения", "accepted": "Выбрана", "rejected": "Отклонена"}
    for task in tasks:
        with st.expander(f"№{task['id']} · {task['title'] or 'Без названия'}"):
            ...
            for p in proposals:
                ...
                a, b = st.columns(2)
                if a.button("Выбрать", key=f"select_{p['id']}", disabled=p["status"] == "accepted"):
                    decide_proposal(p["id"], "accepted")
    ...
    if ask or save_draft or confirm:
        ...
        if ask:
            ...
            if task_id is None:
                ...
                update_task(task_id, {k: v for k, v in candidate.items() if k != "confirmed"})
                if confirm:
