...
class IntegrationTest(unittest.TestCase):
    ...
    def test_full_journey_and_multiple_teams(self):
        ...
        db.update_task(task_id, {"users": task["users"]})
        self.assertFalse(db.get_task(task_id)["confirmed"])
        self.assertEqual(get_catalog(), [])
        db.confirm_task(task_id)
        db.publish_task(task_id)
        ...
        self.assertTrue(all(p["status"] == "accepted" for p in db.get_proposals(task_id)))
    ...
    def test_edit_draft_removes_publication_without_losing_record(self):
        ...
        tid = db.save_task(task)
        db.publish_task(tid)
        db.update_task(tid, {"need": "Дополненная потребность"})
        self.assertEqual(get_catalog(), [])
        self.assertEqual(len(db.get_tasks()), 1)
